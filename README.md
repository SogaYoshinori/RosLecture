...
①シミュレーションをするためにgazeboを実行する．
②ナビゲーションを実行する．
③移動場所を指定するためのrvizを実行する．
④Rvizでの，４輪ロボット(Roboto)と地図データ(Map)と経路()を設定する．
⑤好きな箇所に矢印を配置し，４輪ロボットのナビゲーションを開始する．

##ターミナルで①〜③の実行
　①〜③までの実行するためのコマンドは以下の通りである．それぞれ別々のターミナルにコマンドを打ち実行する(ターミナル数：3)．
